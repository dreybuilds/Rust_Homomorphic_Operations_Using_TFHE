...
fn parallel_homomorphic_operations(mode: ExecutionMode) -> Result<(), Box<dyn std::error::Error>> {
    // Customize the configuration with security and error probability constraints
    let custom_params = tfhe::shortint::parameters::PARAM_MESSAGE_2_CARRY_2;
    
    let config = ConfigBuilder::with_custom_parameters(custom_params, None).build();
